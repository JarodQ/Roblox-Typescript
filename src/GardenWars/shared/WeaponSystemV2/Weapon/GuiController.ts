...
class GuiController {
    ...
    private existingBillboard?: BillboardGui;
    ...
    private createBillboard(
        ...
        this.existingBillboard = billboard; // after creating and parenting it
    ...
    public newBillboard(
        ...
        for (const [, { humanoid, isCritical }] of pairs(targets)) {
            ...
            const finalDamage = (() => {
                if (!this.existingBillboard) return isCritical ? damage.critical : damage.normal;

                const label = this.existingBillboard.FindFirstChildOfClass("TextLabel");
                const stacked = label
                    ? (isCritical ? damage.critical : damage.normal) + (tonumber(label.Text) ?? 0)
                    : isCritical ? damage.critical : damage.normal;

                this.existingBillboard.Destroy();
                this.existingBillboard = undefined;
                return stacked;
            })();
